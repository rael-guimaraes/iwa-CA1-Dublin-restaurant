...
function highlightMealOfTheDay(idTable, bShowMeal) {
    // if bShowVeg is true, then we're highlighting vegetarian
    //	meals, otherwise we're unhighlighting them.
    var i = 0;
    var oTable = document.getElementById(idTable);
    ...
    // "vegetarian" attribute on it.
    for (i = 0; i < aTRs.length; i++) {
        if (aTRs[i].getAttribute('Special_of_the_day') && aTRs[i].getAttribute('Special_of_the_day') == "true") {
            if (bShowMeal) {
                aTRs[i].style.backgroundColor = "lightGreen";
            } else {
                aTRs[i].style.backgroundColor = "";
...
window.addEventListener("load", function () {
    ...
    document.querySelector("#showMeal").addEventListener("click", function () {
        highlightMealOfTheDay('menuTable', this.checked);
    });
});